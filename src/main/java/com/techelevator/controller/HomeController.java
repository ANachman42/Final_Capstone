...
public class HomeController {
	...
	public String postNewFlashcard(HttpSession session, RedirectAttributes redirAttr, @RequestParam String frontText,
			@RequestParam String backText, @RequestParam(required=false) int[] tagId, @RequestParam(required=false) int[] deckId,
			@RequestParam String form_btn) {
		// Validation
		boolean isValid = true;
		if (frontText.length() < 1 || backText.length() < 1 || tagId == null || deckId == null) {
			isValid = false;
	...
	public String updateFlashcard(HttpSession session, RedirectAttributes redirAttr, @RequestParam int cardId,
			@RequestParam String frontText, @RequestParam String backText, @RequestParam(required=false) int[] tagId,
			@RequestParam(required=false) int[] deckId, @RequestParam String form_btn) {
		// Validation
		boolean isValid = true;
		if (frontText.length() < 1 || backText.length() < 1 || tagId == null || deckId == null) {
			isValid = false;
